...
try {
    ...
    if (!isset($data['id']) || !isset($data['title']) || !isset($data['passing_score'])) {
    ...
    if (!$course || $course['user_id'] != $user_id) {
        throw new Exception("You don't have permission to update this quiz");
    }
    ...
    try {
        // Update quiz
        $query = "UPDATE quizzes SET title = ?, description = ?, passing_score = ? WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param(
            "ssii",
            $data['title'],
        ...
        // Get the updated quiz with questions and answers
