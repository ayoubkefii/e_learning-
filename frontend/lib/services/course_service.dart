...
class CourseService {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (response.data is Map && response.data['records'] is List) {
          print('DEBUG: Raw course records: ${response.data['records']}');
          final courses = (response.data['records'] as List).map((json) {
            print('DEBUG: Processing course JSON: $json');
            final course = Course.fromJson(json);
            print('DEBUG: Parsed course: ${course.toJson()}');
            return course;
          }).toList();
          return courses;
        }
  ...
  Future<Course> createCourse(Course course, {PlatformFile? imageFile}) async {
    try {
      print('CourseService: Creating course with data: ${course.toJson()}');
      Response response;
      if (imageFile != null) {
        FormData formData = FormData.fromMap({
          ...
          'image': MultipartFile.fromBytes(imageFile.bytes!,
